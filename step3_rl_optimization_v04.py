#!/usr/bin/env python3
"""
step3_rl_optimization_v04.py
LDPE Tubular PFR — RL Operating Condition Optimisation  v04

Changes from step3_rl_optimization.py:
  v04-1  A_ktrm = 1550: synced with validated step2_rxn_w_T_effect_v03.
         Previous code used A_ktrm = 1.5 (v02 value, 1000× too low). This inflated
         Mn by ~100× (DPn ∝ 1/Cm_eff, Cm_eff = ktrm/kp), making the Mn reward
         band [30–150 kg/mol] impossible to satisfy during training.
         At 195 °C with A_ktrm=1550: Cm_eff ≈ 2.5e-4 → DPn ≈ 3700 → Mn ≈ 104 kg/mol ✓

  v04-2  PDI reward redesigned: single-zone free-radical MOM with equal ktc/ktd
         gives PDI ≈ 1.7–2.1, which is physically correct (not a model defect).
         Industrial LDPE PDI 3–15 comes from multi-zone MWD blending and
         high-conversion LCB — neither is captured in a single-zone MOM.
         Old reward target [3–15] was physically unreachable and penalised every run.
         New scheme:
           +5  PDI ∈ [1.5, 2.5]  — physically expected for one zone
            0  PDI ∈ (2.5, 4.0]  — LCB-enhanced tail, acceptable
           −3  PDI < 1.5          — numerical artifact, penalise
           −2× PDI > 4.0          — model breakdown, penalise proportionally

  v04-3  Warm start: each training episode begins from the final operating
         condition (OC) of the previous episode rather than a fresh random draw.
         Re-randomisation every N_RESET episodes prevents stagnation in one
         basin. Episode 0 always starts randomly.

Algorithm  : REINFORCE + moving-average baseline + Adam
Policy     : 2-hidden-layer MLP (pure NumPy, no framework required)
Environment: Fast LDPE PFR (N=20 spatial nodes, Radau, loose tolerance)

State  (8-D): [X, T_peak*, Mn*, PDI*, T0*, log_ini*, Tc*, U*]  (* normalised)
Action (4-D): [δT₀, δlog(ini₀), δTc, δU]  tanh-squashed continuous

Physical model: L=1500 m, v=10 m/s → τ_res=150 s (full 3-zone LDPE reactor)
  Per-zone targets (Brandolin 1996; Kim & Choi 1988; Kiparissides 2005):
    X per zone : 5–15 %          T_peak : 200–300 °C
    Mn         : 30–150 kg/mol   Mw     : 100–500 kg/mol
    PDI (MOM)  : 1.5–2.5  (single-zone; multi-zone blending not modelled)
"""
...
# =============================================================================
# A.  PHYSICAL & KINETIC CONSTANTS  (v04-1: A_ktrm corrected to 1550)
# =============================================================================
R_gas   = 8.3145
Mw_mono = 28.054
f_eff   = 0.8
...
v_flow  = 10.0        # m/s  →  τ_res = 150 s at L=1500 m
v_cool  = 1.0
...
L_react = 1500.0      # m  (full 3-zone industrial reactor)
NV      = 10

# Arrhenius — v03 calibrated (step2_rxn_w_T_effect_v03.py)
# v04-1: A_ktrm raised from 1.5 (v02, wrong) to 1550 (v03, validated).
A_kd   = 3.15e15;  Ea_kd   = 155_000.0
A_kp   = 6.58e4;   Ea_kp   =  29_500.0
A_ktc  = 2.0e5;    Ea_ktc  =   5_000.0
A_ktd  = 2.0e5;    Ea_ktd  =   5_000.0
A_ktrm = 1550.;    Ea_ktrm =  47_000.0   # v04-1: was 1.5 in RL v03
...
# =============================================================================
# C.  REWARD  (v04-2: PDI target redesigned for single-zone MOM reality)
# =============================================================================
def compute_reward(X, Tpk, Mn, Mw, PDI, ok):
    """
    Multi-objective reward for LDPE single-injection-zone.

    v04-2 PDI change: single-zone free-radical MOM with ktc ≈ ktd gives
    PDI ≈ 1.7–2.1 (physically correct). Industrial PDI 3–15 requires
    multi-zone blending; the old target [3–15] penalised every feasible run.
    New PDI reward reflects what the model can actually achieve.
    """
    if not ok:
        return -100.
    ...
    # --- PDI  (v04-2) ---
    # Single-zone MOM: PDI ≈ 1.7–2.1 (correct); old target [3–15] unreachable.
    if 1.5 <= PDI <= 2.5:
        r += 5.                           # physically expected range
    elif PDI < 1.5:
        r -= 3.                           # numerical artifact
    elif PDI <= 4.0:
        pass                              # LCB-enhanced tail, acceptable
    else:
        r -= 2. * (PDI - 4.0) / 4.0      # model breakdown
...
class GaussianMLP:
    ...
    log_std is a global learnable parameter (state-independent).
    Optimised with Adam.
    """
    ...
    def pg_grad(self, s, u, advantage):
        """∂ log π_Gaussian(u|s,θ) / ∂θ scaled by advantage."""
        mu, ls = self.forward(s)
        std    = np.exp(ls)
...
#     v04-3: warm start — each episode begins from the previous episode's
#     final OC rather than a fresh random draw.  Periodic re-randomisation
#     every N_RESET episodes prevents the trajectory from stagnating.
# =============================================================================
N_EPS   = 150
N_STEPS = 6
N_RESET = 25    # re-randomise starting OC every N_RESET episodes
GAMMA   = 0.95
EMA_A   = 0.10
...
ep_rewards  = []
ep_best_log = []
ep_was_reset = []        # record which episodes were re-randomised
baseline    = 0.
best_r      = -1e9
best_oc     = None
best_result = None
...
print("=" * 68)
print("  LDPE PFR — RL Optimisation  v04  (REINFORCE + warm start)")
print("=" * 68)
print(f"  v04-1  A_ktrm = 1550  (synced to step2_v03; was 1.5 in RL v03)")
print(f"  v04-2  PDI reward: target [1.5–2.5] (single-zone MOM; was [3–15])")
print(f"  v04-3  Warm start: next episode begins from previous final OC")
print(f"         Re-randomise every {N_RESET} episodes to prevent stagnation")
...
# v04-3: initialise OC once before the loop; it is carried forward across episodes.
oc = random_oc()

for ep in range(N_EPS):
    # v04-3: periodic re-randomisation — escape stagnation every N_RESET episodes.
    # Episode 0 already starts from random_oc() above.
    ...
    for step in range(N_STEPS):
        ...
        oc = oc_new   # warm start: carry to next step AND next episode start

    # Discounted returns + REINFORCE update
    G = 0.; returns = []
...
# =============================================================================
# G.  GREEDY EXPLOITATION  — deterministic policy from multiple random starts
# =============================================================================
...
fig.suptitle(
    'LDPE PFR — RL Optimisation v04  (warm start, A_ktrm=1550, PDI target fixed)',
...
plt.tight_layout()
plt.savefig('rl_optimization_v04_results.png', dpi=150)
print("\n  Plot saved → rl_optimization_v04_results.png")
