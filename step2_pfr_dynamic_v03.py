...
# Version 03: Physically realistic cooling parameters
#
# Changes from v02:
#   1. D      : 0.05 → 0.02 m        (벽면적/부피 비 6.25배 → 냉각 강화)
#   2. U_heat : 400  → 2000 W/(m²·K) (냉각 성능 5배 향상)
#   3. Tc_in  : 140  → 60 °C         (냉각 구동력 대폭 증가)
#   4. ini_0  : 1.0  → 0.1 mol/m³    (개시제 농도 조정 → 반응대 분산)
#
# 개선 근거:
#   h_r = 4U/(ρ·Cp·D):
#     v02: 4·400  /(600000·1.7·0.05) = 0.031 s⁻¹
#     v03: 4·2000 /(600000·1.7·0.02) = 0.392 s⁻¹  (12.6배)
#
#   냉각 구동력 (T_reactor - Tc):
#     v02 Tc_in=140°C → T_0=150°C이면 초기 ΔT=10 K (냉각력 극히 부족)
#     v03 Tc_in= 60°C → ΔT ≥ 90 K 확보
#
# 목표:
#   T_peak ≈ 200–270 °C, Conversion ≈ 15–30 %, PDI ≈ 2–10
# =============================================================================
...
L        = 500.0
D        = 0.02            # m  (v02: 0.05 m)
D_j      = 0.04            # m  jacket outer diameter (concentric)
A_r      = np.pi / 4 * D**2
...
Cp       = 1.7
dH_p     = -93_000.0
U_heat   = 2000.0          # W/(m²·K)  (v02: 400)
Tc_in    = 60.0 + 273.15   # K  (v02: 140 °C)
rho_c    = 900_000.0
Cp_c     = 4.18

# 냉각 계수 (inner tube surface 기준)
h_r = 4.0 * U_heat / (rho * Cp * D)
h_j = U_heat * np.pi * D / (rho_c * Cp_c * A_c)

tau_res = L / v
print(f'h_r = {h_r:.4f} s⁻¹  (v02 was 0.0314 s⁻¹)')
print(f'h_j = {h_j:.4f} s⁻¹')
print(f'τ   = {tau_res:.1f} s  (residence time)')
print(f'ΔTad (full conv.) = {(-dH_p)*2.005e4/(rho*Cp):.0f} K')
...
print(f'Jacobian: {N*NV}×{N*NV}, nnz={jac_sp.nnz} ({100*jac_sp.nnz/(N*NV)**2:.2f}% dense)')

# %%
# ---- Inlet Conditions (ini_0 변경) ----------------------------------------
# %%
mono_0 = 2.005e4
ini_0  = 0.1              # mol/m³  (v02: 1.0)
T_0    = 150.0 + 273.15

# %%
# ---- Variable-Specific atol -----------------------------------------------
# %%
...
fig1.suptitle(
    f'LDPE PFR v03 — Steady-state profiles  (t={sol.t[-1]:.0f} s, N={N})\n'
    f'D={D*100:.0f}cm, U={U_heat:.0f}W/m²K, Tc_in={Tc_in-273.15:.0f}°C, ini₀={ini_0}mol/m³',
    fontsize=10
)
...
ax.axhline(Tc_in-273.15, color='b', lw=0.8, ls=':', alpha=0.5, label=f'Tc_in={Tc_in-273.15:.0f}°C')
...
axes1[0, 3].axhline(0, color='k', lw=0.8, ls='--', alpha=0.5)
...
plt.tight_layout()
plt.savefig('ldpe_pfr_v03_steady.png', dpi=150)
...
fig2.suptitle(f'Dynamic evolution  (N={N}, v03)', fontsize=11)
...
plt.tight_layout()
plt.savefig('ldpe_pfr_v03_dynamic.png', dpi=150)

# %%
# ---- Figure 3: 2-D colormaps ----------------------------------------------
# %%
fig3, axes3 = plt.subplots(1, 3, figsize=(16, 4))
fig3.suptitle(f'Spatio-temporal maps  (N={N}, v03)', fontsize=11)
...
plt.tight_layout()
plt.savefig('ldpe_pfr_v03_spacetime.png', dpi=150)
...
print(f'  T_reactor (peak)  : {T_s[-1].max()-273.15:.1f} °C  at z={z[np.argmax(T_s[-1])]:.0f} m')
print(f'  Tc (z=0, outlet)  : {Tc_s[-1,0]-273.15:.1f} °C')
print(f'  T−Tc sign flip    : {"YES — coolant hotter than reactor at some point!" if (T_s[-1]-Tc_s[-1]).min() < 0 else "None (T > Tc everywhere)"}')
